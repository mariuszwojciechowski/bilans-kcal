...
## Pochodne naprawy czerwonego CI z 2026-09-06 („dziś" ze strefy użytkownika, nie procesu)

Kontekst: DONE.md „Testy: «dziś» ze strefy użytkownika, nie zegara runnera".
Sam hotfix jest zrobiony; poniżej to, co z niego wynika, a nie weszło.

- **`date.today()` w kodzie appki poza `clock`** (2/10):
  `app/services/calibration.py:117,208,305` (`period_end = date.today() - 1`
  — na VM w GCP bez ustawionej strefy = UTC, więc między 00:00 a 02:00 CEST
  „wczoraj" kalibracji to dla użytkownika przedwczoraj),
  `app/routers/settings.py:162`, `app/routers/profile.py:45` (tylko rok —
  niegroźne). Przejrzeć każde, przepuścić przez `clock.user_today(profile)`
  tam, gdzie chodzi o dzień użytkownika; `usage.py` zostaje na dniu serwera
  (statystyki admina, jedna strefa dla wszystkich — świadomie).
- **Strażnik w testach** (1/10): test, który grepuje `tests/*.py` (poza
  `test_timezone.py` i `conftest.py`) pod kątem `date.today()` w plikach
  używających `TestClient`, i pada z komunikatem „użyj `app_today()`".
  Tani, a zamyka tę klasę awarii na stałe.
- **Deterministyczny zegar w testach** (4/10): autouse fixture zamrażająca
  czas przez jedno źródło (`clock.now_utc()`), po przepięciu na nie
  `sync/calibration/usage` — wtedy `app_today()` staje się stałą, a suita
  przestaje zależeć od godziny uruchomienia w ogóle (dziś stała `TODAY`
  w `test_day_trends_services.py` liczona przy imporcie może się rozjechać
  z serwerem, jeśli suita przejdzie przez północ warszawską).
...
## Podmiana ikony krasnala z prawdziwej grafiki 24×24 (2/10)

„Ikona krasnala przy komunikatach" (patrz DONE.md, 24.3.0) działa dziś na
fallbacku `icon-192.png` (ikona PWA z rowerem, `width:20px`). Ma ją zastąpić
głowa krasnala z `data/krasnal-icon-source.png`. Plik źródłowy **już leży
lokalnie** (rekonesans 2026-09-06: 2816×1536 RGBA, ale alfa = 255 wszędzie —
czarna kreska na białym tle, bez przezroczystości; sama grafika zajmuje bbox
≈ (1044, 199)–(1832, 1339), czyli 788×1140 px, proporcje ~0,7). Nie czekamy
na „dostarczenie wyciętej grafiki" — wycięcie jest częścią tego zadania,
robi je skrypt. Katalog `data/` jest w `.gitignore`, więc do repo trafia
wyłącznie wynik, a skrypt jest jedynym zapisem, jak ten wynik powstał.

**Decyzje (wiążące, ustalone przy planowaniu 2026-09-06):**

1. **Nazwa pliku zostaje `app/static/krasnal-24.png`**, ale „24" oznacza
   rozmiar CSS, nie bitmapy. Bitmapa ma **72×72 px** (@3×) — ikona 24×24
   wyświetlana na Retinie (DPR 2–3) byłaby rozmyta, a cienka kreska brody
   przy 24 px fizycznych zamienia się w szarą plamę. Przeglądarka skaluje
   72 → 20/24 px sama i robi to lepiej niż my raz na zawsze.
2. **Kolor ikony podąża za kolorem tekstu** — `mobile.html` ma tryb ciemny
   (`prefers-color-scheme: dark`, tło `#131715`), a czarna kreska na nim
   znika. Zamiast dwóch plików (jasny/ciemny) plik jest **czystą maską alfa**
   (RGB czarne, informacja tylko w kanale A), a kolor nadaje CSS:
   `<span class="krasnal-ico">` z `background: currentColor` i
   `mask-image` / `-webkit-mask-image: url(/static/krasnal-24.png)`.
   Komunikaty siedzą w `.muted` (`var(--stone)`), więc ikona jest tego samego
   szarego co tekst w obu motywach, bez filtrów i bez drugiego pliku.
   Fallback, gdyby `mask` sprawiał problemy na testowym telefonie (nie
   powinien — `-webkit-mask` jest w Safari od lat, unprefixed w Chrome 120+):
   zostać przy `<img>` i dodać w bloku dark `filter: invert(.9)`.
3. **Ikona jest dekoracyjna** — `aria-hidden="true"`, bez `alt`; czytnik
   ekranu czyta sam tekst komunikatu, dokładnie jak dziś przy `alt=""`.
4. **Bump `Z`** (kolejny wolny patch po 24.3.0 — jeśli wcześniej wejdzie
   hotfix testów z góry tej listy, to 24.3.2): kosmetyka w istniejącej
   sekcji (VERSIONING.md, „poprawka UI/UX w ramach istniejącej sekcji").

**Kroki dla implementującego LLM:**

1. **Skrypt `scripts/make_krasnal_icon.py`** (wzorzec i styl:
   `scripts/extract_logo.py`, ta sama biblioteka Pillow — jest zależnością
   runtime w `pyproject.toml`, nic nie doinstalowujemy). Użycie:
   `.venv/bin/python scripts/make_krasnal_icon.py [ścieżka_źródła]`,
   domyślnie `data/krasnal-icon-source.png`; brak pliku → czytelny
   `SystemExit` z informacją, że źródło jest poza repo. Działanie:
   - `convert("L")`, maska tuszu = piksele `< 128` (kreska jest czysto
     czarna, tło czysto białe — próg nie jest wrażliwy), `getbbox()` maski;
   - wycięcie bboxu, **dopełnienie do kwadratu** przezroczyście (grafika
     jest pionowa ~0,7 — bez tego `contain` w masce i tak by ją wyśrodkował,
     ale kwadrat upraszcza test i ewentualne inne użycia) plus margines 4 %
     boku;
   - **pogrubienie kreski przed skalowaniem**: `ImageFilter.MaxFilter(size)`
     na masce w pełnej rozdzielczości, `size` nieparzysty rzędu 9–15 (przy
     skali ~1200 → 72 px, czyli ~17×, kreska szerokości 1 px źródła znika;
     `MaxFilter(11)` daje po zeskalowaniu kreskę ~0,7 px, czytelną). Wartość
     jest parametrem skryptu (`--stroke`), dobrana wizualnie w kroku 2 —
     wpisać wybraną jako domyślną z komentarzem „dlaczego";
   - `resize((72, 72), Image.LANCZOS)` maski → kanał A; RGB stałe czarne
     `(0, 0, 0)`; zapis `app/static/krasnal-24.png` (`optimize=True`);
   - **podgląd kontrolny** do `scratch`/`/tmp` (poza repo): kompozyt
     z ikoną w 20 px i 24 px na tle `#F9FAF8` i `#131715`, pokolorowaną na
     `#6C757D` / `#9AA39D` (wartości `--stone` z `mobile.html`), żeby
     ocenić czytelność bez odpalania serwera. Ścieżkę podglądu wypisać.
2. **Dobór grubości kreski** — uruchomić skrypt dla 2–3 wartości `--stroke`,
   obejrzeć podglądy (Read na PNG), wybrać: rozpoznawalna czapka z pomponem,
   oczy/nos jako plamy, broda jako jednolity kształt — **nie** próbować
   zachować kresek brody, przy 20 px to niemożliwe. Jeśli po `MaxFilter`
   twarz się zlewa w plamę, zejść z grubością; jeśli czapka ginie, wejść wyżej.
   Zapisać wybraną wartość jako domyślną skryptu.
3. **`app/templates/mobile.html`:**
   - CSS `.krasnal-ico` (dziś linia ~69): `display: inline-block; width: 20px;
     height: 20px; vertical-align: -4px; margin-right: 6px; background:
     currentColor; -webkit-mask: url(/static/krasnal-24.png) center / contain
     no-repeat; mask: url(/static/krasnal-24.png) center / contain no-repeat;`
     — rozmiar i wyrównanie zostają jak dziś (były dobrane pod linię tekstu);
   - `krasnalSays()` (~linia 490): `'<img class="krasnal-ico"
     src="/static/icon-192.png" alt="">'` → `'<span class="krasnal-ico"
     aria-hidden="true"></span>'`;
   - komentarz nad `KRASNAL_STATUS` (~linie 476–478) mówi o fallbacku
     „dopóki właściciel nie dostarczy" — **usunąć to zdanie**, zostawić
     wskazanie, że grafikę generuje `scripts/make_krasnal_icon.py`.
   `icon-192.png` **zostaje** w `app/static/` — używa go
   `manifest.webmanifest`; usuwamy tylko odwołanie z `krasnalSays()`.
4. **Test `tests/test_krasnal_icon.py`** — rozszerzyć, nie tylko podmienić
   ścieżkę:
   - `GET /static/krasnal-24.png` → 200 i `content-type: image/png`;
   - plik otwarty Pillow: rozmiar `(72, 72)`, tryb z alfą, **narożniki
     przezroczyste** (`A == 0` w czterech rogach) i **środek nieprzezroczysty
     gdzieś w kolumnie środkowej** (jest tusz) — pilnuje, że ktoś nie wrzucił
     przez pomyłkę pliku z białym tłem albo pustej maski;
   - **ścieżka w szablonie zgodna z plikiem**: wczytać
     `app/templates/mobile.html` jako tekst, `re.findall(r"/static/krasnal-[\w.@-]+\.png", …)`,
     dla każdego trafienia `GET` → 200 i asercja, że zbiór trafień jest
     niepusty (dziś test sprawdza `icon-192.png` na sztywno, co nie chroni
     przed rozjazdem ścieżki w JS/CSS i nazwy pliku — to jest ryzyko, którego
     ten test miał pilnować). Odpalić `pytest tests/test_krasnal_icon.py` na
     bieżąco; pełna suita dopiero za zgodą właściciela (zasada wyżej).
5. **Weryfikacja ręczna** na dev serwerze (launch config `fit-krasnal`,
   port 8321, osobne `FIT_KRASNAL_DATA`, konto testowe skasować po
   sprawdzeniu — jak przy 24.3.0): ikona przy quipie na „Dziś" i przy
   „Krasnal nie ma danych…" (konto bez profilu), w **obu motywach**
   (`resize_window` z `colorScheme: light` / `dark`), bez błędów w konsoli,
   bez 404 na `/static/krasnal-24.png` w logach serwera. Zrzut ekranu jasny
   i ciemny do wpisu w DONE.md (opis słowny wystarczy, plików nie commitować).
6. **`VERSION`** — bump `Z` wg decyzji 4. Komunikat commita w stylu
   poprzednich („Ikona krasnala z prawdziwej grafiki — maska 72×72, kolor
   z CSS (24.3.x)").
7. **Statystyki: świadomie brak** — zmiana czysto wizualna, bez nowej
   funkcji do mierzenia adopcji (ikona pokazuje się każdemu, kto widzi
   komunikat; to nie jest ficzer z opt-in). Jedyny sensowny sygnał
   funkcjonowania to brak błędów 404 na `/static/krasnal-24.png` w logach
   uvicorn po deployu (`journalctl -u fit-krasnal | grep krasnal-24`) —
   sprawdza właściciel po pierwszym wejściu testerów, bez licznika w
   `usage.py`. Ten sam wybór co przy 24.3.0 (DONE.md: „Bez statystyk").
8. **Nota `/prywatnosc` bez zmian** (nic nowego nie zbieramy).
9. **TODO/DONE**: sekcję przenieść do DONE.md z opisem decyzji 1–2 (dlaczego
   72 px, dlaczego maska zamiast dwu plików) i wybraną wartością `--stroke`;
   w dotychczasowym wpisie DONE „Ikona krasnala przy komunikatach" **nie
   poprawiać** zdania o fallbacku — to zapis historyczny, nowy wpis go
   domyka odsyłaczem.

Poza zakresem (nie dotykać): `manifest.webmanifest`, `apple-touch-icon`,
`favicon-32.png`, `logo.png` w nagłówku, `deploy/landing/index.html` — te
używają znaku z rowerem (`scripts/extract_logo.py`) i mają zostać, dopóki
właściciel nie zdecyduje o zmianie całego brandingu (osobny punkt, jeśli
kiedyś wróci).
