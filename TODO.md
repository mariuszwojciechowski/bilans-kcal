...
## Trendy liczą kcal inaczej niż „Dziś" — jedna logika wydatku w całej aplikacji (4/10)

**Bug zgłoszony 2026-09-04.** Na ekranie „Dziś" bilans bieżącego dnia jest
zielony (spożyte < spalone), a w „Trendach" ten sam dzień ma czerwony słupek
i na wykresie „Energia" spożyte leżą wyżej niż spalone. Dni minione zgadzają
się, o ile nie było w nich ręcznej aktywności ani braku wpisu Garmina.

**Diagnoza.** „Dziś" (`app/services/day.py:day_report()`) liczy wydatek przez
`balance.day_balance()`, który ma trzy gałęzie: dzień domknięty →
Garmin + kcal aktywności ręcznych; dzień w toku → `max(Garmin + ręczne,
model TDEE)`, żeby nie zaniżać wydatku, zanim zegarek dośle dane; brak wpisu
Garmina → model TDEE. „Trendy" (`app/services/trends.py:payload()`, linia
`kcal_out = [(s.date, float(s.kcal_total_garmin)) …]`) biorą **surowe**
`DailySummary.kcal_total_garmin` i nic więcej — bez modelu, bez aktywności
ręcznych, bez rozróżnienia dnia w toku. Stąd trzy rozjazdy:

| Sytuacja | „Dziś" | „Trendy" |
|---|---|---|
| dzień w toku (Garmin dosłał np. 1300 kcal, model mówi 2600) | 2600 (`mixed`, szacowane) | 1300 → fałszywa nadwyżka |
| dzień domknięty z aktywnością ręczną | Garmin + ręczne | sam Garmin |
| dzień bez wpisu Garmina (użytkownik bez zegarka) | model TDEE | dzień pominięty na obu wykresach |

**Decyzje (wiążące, ustalone z właścicielem 2026-09-04):**

- **Jedna funkcja wydatku dla całej aplikacji.** Wydatek dnia — dzisiejszego
  i minionego — liczy wyłącznie jedno miejsce; „Dziś", wykres „Energia",
  wykres „Bilans dzienny" i średni bilans biorą z niego tę samą liczbę.
  Żaden widok nie czyta `kcal_total_garmin` bezpośrednio.
- **Dzień w toku w Trendach jest szacowany dokładnie tak jak na „Dziś"**
  (gałąź `mixed`/`model` z `day_balance`, flaga `estimated=True`) i **ma
  inny kolor słupka** niż dni domknięte. Ten sam kolor „szacowany" dostają
  dni minione bez wpisu Garmina (model) — jedno pojęcie „szacowane" w całej
  aplikacji, zgodne z flagą `estimated` z `/api/day/{day}`.
- **Wykres „Energia" dla dzisiaj bierze te same wartości co bilans**
  (spożyte = suma posiłków, spalone = wynik wspólnej funkcji), punkt dnia
  szacowanego rysowany inaczej (pusty okrąg). Etykieta „spalone (Garmin)" →
  „spalone", bo wartość bywa Garmin, model albo mix.
- **Waga do modelu TDEE = ta sama, którą bierze „Dziś"** — `smoothed_weight`
  z pełnej historii (ostatnie 7 dni od najnowszego pomiaru), dla każdego dnia
  zakresu. `day_report(day)` dla dnia minionego też tak robi, więc to jest
  jedyna wersja spójna z „Dziś". Waga „z tamtego dnia" byłaby dokładniejsza,
  ale to osobny punkt (dotknie także `day_report`), nie ten.
- **Bilans tylko dla dni z ≥ 1 zapisanym posiłkiem** — bez zmian względem
  dziś. Dzień bez posiłków to „nie wiem", nie „0 kcal"; „Dziś" pokazuje
  wtedy bilans, bo użytkownik dopiero zaczyna dzień. Wykres „Energia"
  natomiast pokazuje spalone również w dniach bez posiłków (gdy jest wpis
  Garmina), jak dotychczas.
- **Średni bilans i prognoza celu liczą się z dni domkniętych** (`estimated
  == False`). Dzień w toku zmienia się co godzinę i zaburzałby średnią;
  `balance_days` w napisie „(N dni z danymi)" liczy tylko te dni. Słupek
  szacowany jest na wykresie, ale nie w średniej. *(Jeśli właściciel
  zdecyduje inaczej przy starcie — jedna linia do zmiany, odnotuj w DONE.md.)*
- **Kontrakt `/api/trends` bez nowych kluczy** — informacja „szacowany" jedzie
  w SVG (kolor/legenda), `tests/test_day_trends_services.py::API_TRENDS_KEYS`
  zostaje bez zmian. Mobile nic nie musi wiedzieć.

**Kroki dla implementującego LLM:**

1. **Czysta funkcja w `app/services/day.py`:** `day_energy(profile, weight_kg,
   day, summary, activities, meals, today) -> DayEnergy` — dataclass z polami
   `kcal_in`, `kcal_out`, `out_source`, `estimated`, `tdee` (`TheoreticalTdee`),
   `manual_kcal`, `activities_kcal`, `steps`, `steps_effective`. Bez dostępu
   do bazy — dostaje już pobrane wiersze, żeby Trendy mogły ją wołać w pętli
   po 180 dniach bez 180 zapytań. Treść to dokładnie dzisiejsze linie
   `day_report` od `kcal_in = sum(...)` do `steps_kcal = ...` (budowa
   `activities_for_tdee`, `manual_kcal`, `steps_effective`, `tdee_theoretical`,
   `day_balance`). `day_complete` = `bool(summary and summary.complete)` —
   jak dziś; `sync.py:47` ustawia `complete = day < today`, więc dla dnia
   wstecz bez synchronizacji od tamtej pory `complete` może być `False` — to
   jest istniejące zachowanie „Dziś", nie naprawiamy tu.
2. **`day_report` woła `day_energy`** i z wyniku buduje `out_breakdown`,
   `balance`, `remaining_kcal` itd. Zero zmiany w kształcie odpowiedzi
   `/api/day/{day}` — `test_api_day_returns_exactly_what_service_computes`
   i `tests/test_activities_api.py` muszą przejść bez modyfikacji (dowód, że
   refaktor nie zmienił liczb).
3. **`trends.payload` przechodzi na `day_energy`.** Trzy zapytania zbiorcze
   za zakres (są już: `summaries`, `meals`; dopisać `activities`), pogrupowane
   po dacie w słowniki, plus `profile` i `weight = smoothed_weight(all_weights)`
   (już pobierane do wygładzania — nie pobieraj drugi raz). Dla każdego dnia
   `start..today`, w którym jest posiłek **lub** wpis Garmina, policz
   `day_energy`. Z wyniku:
   - `kcal_out` (seria „spalone") = `e.kcal_out` dla każdego takiego dnia,
   - `balance` = `e.kcal_in - e.kcal_out` tylko dla dni z ≥ 1 posiłkiem,
   - zbiór `estimated_days = {d for d, e in ... if e.estimated}`,
   - `avg_balance`, `balance_days`, `goal_eta` — z dni **poza**
     `estimated_days`.
   Bez profilu (`profile is None`) albo bez wagi zostaje dzisiejsze
   zachowanie: wydatek tylko z Garmina, bez modelu — Trendy nie mogą rzucić
   `DayReportUnavailable`, bo wykres wagi/posiłków ma się nadal wyświetlić.
   Zapisz to w docstringu jako świadomy fallback.
4. **`charts.bar_chart` — słupki szacowane.** Nowy parametr
   `estimated: set[date] = frozenset()` i dwa kolory: dla dnia w zbiorze
   ten sam odcień (czerwony/zielony) z `fill-opacity=".45"` i przerywanym
   obrysem `stroke` w kolorze pełnym (`stroke-dasharray="3,2"`), żeby na
   telefonie różnica była widoczna także w jasnym słońcu, nie tylko przez
   nasycenie. Pod osią legenda w stylu `line_chart`: pełny kwadrat „dzień
   domknięty", półprzezroczysty „szacowany (dzień w toku lub brak pomiaru)".
5. **`charts.line_chart` — punkty szacowane.** `Series` dostaje opcjonalne
   `hollow: set[date]`; punkt z tego zbioru rysowany jako okrąg
   `fill="white" stroke=color stroke-width="1.5"` zamiast pełnego. Trendy
   podają `hollow=estimated_days` dla serii „spalone". Etykieta serii:
   „spalone".
6. **Teksty.** `trends.html:104` „Tylko dni z zapisanymi posiłkami i
   pomiarem Garmina." → „Dni z zapisanymi posiłkami. Jaśniejszy słupek =
   wydatek szacowany (dzień w toku albo brak pomiaru Garmina) — tak samo jak
   na «Dziś»." Analogiczna linia pod wykresem bilansu w `mobile.html` (blok
   `trends-charts`, ok. linii 917). Legenda w SVG i tekst mówią to samo
   słowo: „szacowany".
7. **Testy `tests/test_day_trends_services.py`** (rozszerzyć, nie nowy plik —
   to test spójności obu widoków):
   - dzień w toku: seed `DailySummary(today, kcal_total_garmin=1300,
     complete=False)` + posiłki 1600 kcal + profil + waga; bilans dzisiejszego
     słupka w `trends_service.payload` **równa się** `day_report(today)["balance"]`
     i jest ujemny (zielony); SVG `chart_balance` zawiera dokładnie jeden
     słupek z `fill-opacity` (dzisiejszy). Nie dodawaj do `payload`
     pomocniczych kluczy „tylko dla testów" — liczby sprawdzaj przez
     `day_energy` na tych samych wierszach, oznaczenie przez tekst SVG,
   - dzień domknięty z aktywnością ręczną 300 kcal: bilans w Trendach =
     `day_report` = posiłki − (Garmin + 300),
   - dzień bez wpisu Garmina: w Trendach jest słupek (model), równy
     `day_report`, oznaczony jako szacowany,
   - `avg_balance` liczy tylko dni domknięte: seed 2 dni domknięte + dziś →
     `balance_days == 2`, średnia bez dnia dzisiejszego,
   - `test_api_trends_shape_is_exactly_the_contract` bez zmian (dowód, że
     kontrakt API nie urósł).
   Nowy `tests/test_charts.py`: `bar_chart` z `estimated={d}` daje
   `fill-opacity` tylko dla tego słupka; `line_chart` z `hollow={d}` daje
   `fill="white"` tylko dla tego punktu; bez tych parametrów SVG identyczny
   jak dziś (snapshot na krótkiej serii).
8. **Wersja i porządek.** `VERSION` 21.3.1 → **21.4.0** (Y: zmiana sposobu
   liczenia bilansu w Trendach — wg VERSIONING.md „ważna zmiana logiki" to
   nawet X; wybieramy Y, bo „Dziś" liczy identycznie jak przed zmianą, zmienia
   się tylko dopasowanie Trendów do niego; w razie wątpliwości właściciel może
   podnieść do 22.0.0). Wpis w DONE.md z tabelą rozjazdów z tej sekcji.
   Nota `/prywatnosc` **bez zmian** — te same dane, inne zestawienie.
9. **Uruchamianie testów wg zasady z nagłówka tego pliku:** w trakcie pracy
   `pytest tests/test_day_trends_services.py tests/test_charts.py
   tests/test_balance.py tests/test_activities_api.py`; pełna suita dopiero
   za zgodą właściciela. Commit po zielonym; **push dopiero po wyraźnej
   zgodzie** — ta zmiana idzie prosto na produkcję do testerów.
