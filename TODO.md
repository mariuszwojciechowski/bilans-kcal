...
## Prognoza doby: spoczynek z historii Garmina zamiast Mifflina (2/10)

**Objaw (dane właściciela, 2026-09-10, `/usage?scope=me`):** prognoza poranna
1755–1896 kcal, a dni **bez treningu** kończą się na 2088–2236 (Garmin). Prognoza
„zwyczajnego dnia" jest ~350 kcal za nisko. Dni z treningiem (2852–3649) są
wyżej celowo — treningi nie wchodzą do bazy, cel rośnie po ich synchronizacji
(decyzja właściciela, **nie zmieniaj**).

**Przyczyna (zweryfikowana):** `app/services/day.py:309` liczy spoczynek do
prognozy jako `max(kcal_bmr_garmin z DZIŚ, Mifflin)`. Garmin podaje spoczynek
**narastająco** (rano 679), więc zawsze wygrywa Mifflin (~1644), a pełnodobowy
spoczynek Garmina to ~2000 (kolumna „BMR (Garmin)" dni domkniętych: 1998–2020).
Reszta dnia liczy się z 1644 zamiast 2000 → ~17 h × 15 kcal/h ≈ 250–350 kcal
za mało. Prognoza ma przewidywać liczbę Garmina, więc ma używać jego spoczynku.

**Decyzja:** `bmr_full` = **mediana `kcal_bmr_garmin` z ostatnich 7 domkniętych
dni** (te same dni, które już pobiera `_baseline_neat`); fallback: Mifflin,
gdy < 3 dni z tą kolumną. Bez `max` z dzisiejszym narastającym BMR. Nic więcej
w prognozie się nie zmienia (NEAT, okno czuwania, godzina synchronizacji).

**Instrukcja dla implementującego LLM — czytaj tylko to (łącznie ~120 linii):**

| Plik | Zakres | Po co |
|---|---|---|
| `app/services/day.py` | 38-41 (stałe), 89-114 (`_baseline_neat`), 300-315 (blok `forecast` w `day_report`), 355-362 (słownik `forecast` w odpowiedzi) | jedyne miejsce zmiany |
| `tests/test_activities_api.py` | 381-422 (test prognozy z 7 dniami historii), 437-450 (fallback bazy) | asercja `bmr_full > 751` zmienia sens; seed już ma `kcal_bmr_garmin=1800` w 7 dniach |

**Nie czytaj:** `energy.py` (`full_day_forecast` dostaje gotowy `bmr_full` —
sygnatura bez zmian), `balance.py`, `trends.py`, `usage.py` (rozkład
„prognoza ÷ pomiar" liczy się z zapisanych kolumn i sam pokaże efekt),
`mobile.html` (równanie na dole Trendów czyta `forecast.bmr_full` — nowa wartość
wejdzie sama), `models.py`, `db.py` (bez nowych kolumn), `DONE.md` poza wpisem.

**Kroki (1 commit):**

1. `_baseline_neat` → zmień nazwę na `_history_baselines` i zwracaj trzy
   wartości: `(neat_kcal, days_used, bmr_garmin_full | None)`, gdzie
   `bmr_garmin_full = median(s.kcal_bmr_garmin for s in summaries if s.kcal_bmr_garmin)`
   przy ≥ `BASELINE_NEAT_MIN_DAYS` takich wierszach, inaczej `None`. Ta sama
   lista `summaries`, zero nowych zapytań. Docstring: jedno zdanie o spoczynku.
2. W bloku `forecast` (`day.py:306-311`): `bmr_full = bmr_garmin_full if
   bmr_garmin_full is not None else e.tdee.bmr`. Usuń `max(...)` i komentarz
   o „narastające/całodobowe" — zastąp jednym zdaniem: „spoczynek z historii
   Garmina, bo prognoza ma trafić w liczbę Garmina".
3. Do słownika `forecast` w odpowiedzi dopisz `"bmr_source": "garmin" | "mifflin"`
   (żeby na dole Trendów było widać, skąd liczba — jedno słowo w
   `renderTargetFormula` **nie** jest wymagane w tym punkcie).
4. Testy (`tests/test_activities_api.py`): w teście 381-422 zamień
   `assert f["bmr_full"] > 751` na `assert f["bmr_full"] == 1800` i
   `f["bmr_source"] == "garmin"` (seed daje 7 dni po 1800); w teście
   fallbacku 437-450 dopisz `assert f["bmr_source"] == "mifflin"`.
   Uruchom **tylko** `pytest tests/test_activities_api.py -q -p no:warnings`.
5. `VERSION` **Z+1** (poprawka istniejącej prognozy). Commit lokalny, bez
   pusha. Wpis w DONE.md: 6–8 linii (objaw, przyczyna, decyzja, „nota
   `/prywatnosc` bez zmian"), ten punkt usuń z TODO.

**Statystyki:** bez nowych — istniejący rozkład „prognoza poranna ÷ pomiar
końcowy" na `/usage` po tygodniu pokaże, czy mediana przesunęła się z ~0,6
w stronę 0,75–0,8 dla dni z treningiem i ~1,0 dla dni bez. Właściciel:
sprawdź to po 7 dniach i dopisz wynik do wpisu w DONE.md.
