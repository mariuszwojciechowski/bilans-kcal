...
## Zakres statystyk `/usage`: testerzy / wszyscy / tylko ja (2/10)

**Problem (właściciel, 2026-09-05):** konto admina (`ADMIN_EMAIL`) jest dziś
**wycięte** ze wszystkich agregatów `/usage` (`dashboard_stats`: filtr
`r[0] != admin_ref` na wierszach telemetrii i podzapytanie `other_users`
dla lejka, posiłków i Garmina). Tymczasem właściciel jest najaktywniejszym
użytkownikiem i jedynym, który dziś ma dane do złapania problemów w rodzaju
„4213 vs 2889 kcal" — statystyki funkcjonowania (punkt niżej) liczone bez
niego są przez pierwsze tygodnie puste. Jednocześnie widok adopcji **przez
innych** nie może być zaciemniony jednym bardzo aktywnym kontem.

**Decyzja:** przełącznik zakresu na `/usage`, trzy wartości:

| `scope` | Kto wchodzi | Do czego |
|---|---|---|
| `others` (**domyślny**, dzisiejsze zachowanie) | wszyscy poza adminem | adopcja przez testerów — bez zaciemnienia |
| `all` | wszyscy | pełny obraz obciążenia i funkcjonowania |
| `me` | tylko admin | „czy apka liczy dobrze u mnie" — tu wolno pokazać **szczegóły per dzień**, bo to własne dane właściciela |

Dane **już są zbierane** dla admina (`bump` nie filtruje) — zmiana jest
wyłącznie w warstwie odczytu. Nota `/prywatnosc` bez zmian (dane innych
użytkowników nadal wyłącznie w agregatach; `me` pokazuje dane właściciela
jemu samemu). Odnotuj to w DONE.md.

**Instrukcja dla implementującego LLM — co czytać:**

| Plik | Zakres | Po co |
|---|---|---|
| `app/services/usage.py` | 101-170 | jedyne miejsca filtrowania admina: linia ~119 (`r[0] != admin_ref`), ~138 (`total_accounts − 1`), ~143 (`other_users`) i wszystkie `.in_(select(other_users.c.id))` niżej |
| `app/routers/usage.py` | 18-27 (`usage_page`) | dopisz `scope: str = "others"` jako query param i przekaż dalej |
| `app/templates/usage.html` | 46-55 (header + akapit „Widok operuje…") | miejsce na trzy linki-zakładki `?scope=…`; podpis akapitu zależny od zakresu |
| `tests/test_usage.py` | 100-116 | wzorzec testu strony admina; **uwaga:** test 106-116 asertuje, że `user_ref(1)` (admin w tym teście) jest w HTML — sprawdź, dlaczego dziś przechodzi mimo filtra, zanim zmienisz semantykę (prawdopodobnie tabela „ostatnia aktywność" albo `ADMIN_EMAIL` w testach ≠ rejestrowany e-mail) |

**Nie czytaj:** `usage.html` poniżej linii 57 (sekcje renderują klucze
słownika — nie zmieniają się), `charts.py`, `models.py`, `consent.py`,
`admin_consents.html`.

**Kroki:**

1. `dashboard_stats(db, weeks=12, scope="others")`. Zwaliduj `scope` w routerze
   (`Literal["others", "all", "me"]`, inne → 422). W serwisie zbuduj **jeden**
   zbiór dozwolonych `user_id` (`allowed_ids`) i jeden zbiór pseudonimów
   (`allowed_refs`) na podstawie `scope`, a wszystkie istniejące filtry
   (`r[0] != admin_ref`, `other_users`, `− 1` przy `total_accounts`) zamień
   na `in allowed_refs` / `.in_(allowed_ids)`. Nie zostawiaj dwu ścieżek.
2. Szablon: trzy linki w headerze obok „Zgody RODO" — `testerzy · wszyscy ·
   ja`, aktywny podkreślony (klasa `.nav` już jest). Akapit pod headerem
   dostaje dopisek: dla `others` bez zmian; dla `all` „…łącznie z kontem
   admina"; dla `me` „Tylko Twoje konto — poniżej także szczegóły per dzień."
3. **Tylko dla `scope == "me"`** dodatkowa sekcja „Moje dni: model vs pomiar"
   (ostatnie 14 dni, tabela): data, `kcal_total_garmin`, `kcal_active_garmin`,
   `kcal_bmr_garmin`, `model_total_kcal` (kolumna z punktu „Statystyki…"
   niżej — jeśli implementujesz ten punkt **przed** tamtym, pokaż `—` i
   zostaw TODO w komentarzu szablonu), liczba aktywności zegarkowych i ile
   z nich ma `kcal_bmr_garmin`. To jest widok, który złapałby 4213 kcal
   pierwszego dnia. Dla `others`/`all` sekcja **nie** renderuje się — dane
   per dzień innych użytkowników nie mają prawa się tu pojawić, nawet pod
   pseudonimem.
4. W tabeli „Ostatnia aktywność per pseudonim" przy `all` oznacz wiersz
   admina dopiskiem „(ja)" — pseudonim admina i tak jest znany adminowi.
5. Testy `tests/test_usage.py`: (a) `others` nie zawiera pseudonimu admina
   w tabeli aktywności, `all` zawiera z „(ja)", `me` zawiera tylko jego;
   (b) `scope=xyz` → 422; (c) sekcja „Moje dni" obecna tylko przy `me`;
   (d) e-mail admina nadal nigdzie w HTML (rozszerz 106-116).
6. Wersja: **Y** (nowy sposób działania istniejącej strony). Commit lokalny,
   bez pusha. Punkt „Statystyki: obserwowalność…" niżej zakłada, że ten
   przełącznik już istnieje — **rób ten punkt pierwszy**, jeśli oba w jednej
   sesji.
