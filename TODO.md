...
## Integracja ze Strava — dla użytkowników bez Garmina (złożoność: 7)

**Kontekst.** Dziś `DataProvider` (`app/providers/__init__.py`) jest już
abstrakcją dostawcy — jedyna implementacja to `GarminProvider`
(`app/providers/garmin.py`), używana na sztywno w `sync.maybe_sync` i w
ręcznej synchronizacji (`profile.py: sync_manual`). Kto nie ma Garmina, dziś
i tak może korzystać z appki: kroki wpisuje ręcznie (`DailySummary.steps` bez
synchronizacji), a aktywności dodaje przez `POST /api/activities` z
`source="manual"` (`app/routers/day.py`, licznik MET w
`energy.manual_activity_kcal`). Strava ma **wyłącznie aktywności** — nie ma
odpowiednika dziennego podsumowania Garmina (`totalKilocalories`,
`bmrKilocalories`, kroki) ani wiarygodnego pomiaru wagi. Integracja ze Stravą
zamienia więc tylko *ręczne dodawanie aktywności* na *automatyczny import*
dla tej grupy użytkowników — kroki i kalorie dnia zostają ręczne, tak jak są
teraz.

**Decyzja o unikaniu duplikatów (wiążąca, żeby nie robić dopasowywania po
czasie/dystansie):** Garmin ma priorytet. Jeśli użytkownik ma połączone
**oba** konta, synchronizujemy tylko z Garmina — Strava jest wtedy całkowicie
ignorowana (typowy przypadek: Garmin automatycznie eksportuje aktywności do
Stravy, więc synchronizacja obu źródeł zawsze dawałaby duplikaty tej samej
aktywności pod dwoma różnymi ID). Kolejność wyboru providera dla usera:
Garmin (jeśli tokeny są) → inaczej Strava (jeśli tokeny są) → inaczej brak
auto-sync (dzisiejszy stan, tylko ręcznie). To jest jedyny mechanizm
...
2. **Provider `app/providers/strava.py`** — implementuje `DataProvider`:
   - `get_daily_summary(day)` — zwraca `DailySummaryData` z samymi `None`
     ...
     usera bez żadnego providera. Nie zgaduj kalorii całodniowych ze Stravy —
     tam ich nie ma.
   - `get_weights(start, end)` — zwraca `[]` (Strava nie jest źródłem wagi
     w tym planie; API zwraca wagę tylko jeśli user ją tam wpisał do liczenia
     mocy rowerowej — zbyt niepewne, żeby na tym polegać).
   - `get_activities(start, end)` — `GET /api/v3/athlete/activities` (paginacja,
     filtr `after`/`before` w epoch), mapowanie na `ActivityData`:
     `garmin_id` = `f"strava-{activity['id']}"` (patrz punkt 4 — reużywamy
     istniejącą kolumnę), `type` = mapowanie `sport_type` Stravy na te same
     stringi co `activity.type` u Garmina, gdzie się da (`Run`→`running`,
     `Ride`→`cycling`, `WeightTraining`→`strength_training`, inne → `other`;
     lista w `energy.py` — trzymaj się tych samych kluczy, bo od `type`
     zależy dobór MET przy... nie, przy Stravie `kcal` mamy z API
     (`calories`), MET nie jest liczony, ale `type` steruje ikoną/etykietą w
     UI, patrz szablony aktywności), `duration_s` = `elapsed_time`,
     `distance_m` = `distance`, `kcal` = `calories` (Strava liczy je tylko dla
     aktywności z danymi z zegarka/HR — może być `None`), `avg_hr` =
     `average_heartrate` (może być `None`, nie każdy ma pas HR), `kcal_bmr` =
     `None` (Strava nie liczy spoczynku), `steps` = `None` dla większości
     typów (dla `Run`/`Walk` czasem jest w detalu aktywności, ale
     endpoint listy go nie ma — nie dociągaj `GET /activities/{id}` per
     aktywność tylko po kroki, to jest N+1 zapytań; zostaw `None`, `day.py`
     ma już fallback `_est_steps` dla aktywności bez kroków).
3. **OAuth i tokeny — wzorzec identyczny z `garmin.py`/`settings_service`**
   (część B planu szyfrowania sekretów, patrz [[fit-krasnal-secrets-encryption]]
   — tokeny Stravy też muszą iść przez `crypto.encrypt`, nie leżeć jawnie):
   - Dodaj `"strava_tokens"` do `SECRET_SETTING_KEYS` w
     `app/services/settings.py`. Wartość: JSON `{"access_token", "refresh_token",
   ...
   - `app/routers/settings.py`:
     ...
     - `GET /settings/strava/callback?code=...` — wymienia `code` na tokeny
       (`POST https://www.strava.com/oauth/token` z `client_id`,
       `client_secret`, `code`, `grant_type=authorization_code`), zapisuje
       blob, `usage_service.bump(db, user.id, "strava_connect_ok")`, redirect
       na `/settings`. Błąd Stravy (np. user kliknął „Odmów") → redirect
       z komunikatem, bez wyjątku 500.
     - `POST /settings/strava/disconnect` — usuwa `strava_tokens`
       (`set_setting(..., None)`); opcjonalnie `POST
       https://www.strava.com/oauth/deauthorize` z aktualnym access tokenem
       (Strava API Agreement wymaga wsparcia deautoryzacji — zrób to przed
       skasowaniem tokenu lokalnie, błąd tego wywołania nie blokuje
       skasowania lokalnego).
   - `app/providers/strava.py`: `_client()` sprawdza `expires_at`; jeśli
     token wygasł (albo wygasa w ciągu <5 min), odświeża przez `POST
     .../oauth/token` z `grant_type=refresh_token`, zapisuje nowy blob od razu
     (Strava rotuje `refresh_token` przy każdym odświeżeniu — stary przestaje
     działać, nie wolno go zostawić w bazie).
4. **Model — reużyj `Activity.garmin_id`, nie dodawaj nowej kolumny.** Kolumna
   nazywa się `garmin_id`, ale już dziś przechowuje `"manual-<uuid>"` dla
   wpisów ręcznych (`app/routers/day.py`), więc jest de facto
   „identyfikator zewnętrzny, provider-specific". Dla Stravy: prefiks
   `"strava-<id>"` (patrz punkt 2). `UniqueConstraint("user_id", "garmin_id")`
   zostaje bez zmian. Rozszerz `source` (dziś `garmin | manual`) o
   `"strava"` — sam string w kolumnie `String`, bez migracji (nie jest
   enumem w bazie). Zaktualizuj komentarz przy `source` w `app/models.py`
   (`garmin | manual` → `garmin | manual | strava`) i przy `garmin_id`
   (dopisz, że dla Stravy trzyma `"strava-<id>"`).
5. **Wybór providera per user — nowa funkcja, zamiast sztywnego
   `GarminProvider(...)` w trzech miejscach.** W `app/providers/__init__.py`
   albo nowym `app/services/provider_select.py`:
   ```python
   def get_provider_for_user(db, user_id) -> DataProvider | None:
       ...
       return None
   ```
   Podmień wywołania w `app/services/sync.py: maybe_sync` (dziś linia z
   `GarminProvider(user_id, db)`) i w `app/routers/profile.py: sync_manual`
   na `get_provider_for_user`; jeśli `None`, zachowanie identyczne do
   dzisiejszego braku Garmina (nic nie synchronizuj, UI zostaje na ręcznym
   wpisywaniu). `sync_range` w `sync.py` nie wymaga zmian — już przyjmuje
   `DataProvider` jako parametr, nie wie że to Garmin.
6. **UI ustawień (`app/templates/settings.html`, `mobile.html` sekcja
   ...
   - Gdy `garmin_connected` jest `True`: karta Stravy wyszarzona/zwinięta
     ...
     że coś jest zepsute, gdy połączy oba i nie widzi żadnej zmiany).
...
7. **Statystyki na `/usage`** (patrz [[feedback-plans-include-stats]] — każdy
   ...
   `"strava_disconnect"`. Bumpuj `strava_sync_ok`/`strava_sync_error` w
   `sync.maybe_sync` analogicznie do dzisiejszego logowania błędu Garmina
   (`logger.warning("Auto-sync Garmin nieudany...")` — dodaj rozróżnienie po
   typie providera w treści loga, np. `"Auto-sync %s nieudany: %s"`). Na
   `/usage` (sprawdź `app/routers/usage.py` albo gdzie renderuje się panel)
   dodaj wiersz „Strava połączona: N userów, ostatnia synchronizacja
...
9. **Testy `tests/test_strava.py`** (wzorzec `tests/test_garmin_sync.py` jeśli
   ...
   zwraca Garmin gdy oba są połączone (test priorytetu — to jest test
   antyduplikacyjny, nie test dopasowywania rekordów); zwraca Strava gdy
...
(`pytest tests/test_strava.py`) puszczaj swobodnie.
