...
def test_manual_activity_kcal():
    """MET calculation: 60 min bieganie intensywne @ 70kg = ~840 kcal"""
    from app.services import activity
    kcal = activity.calculate_kcal("bieganie", "intensywna", 60, 70)
    assert kcal == 840  # 12.0 MET * 70 kg * 1 h = 840 kcal


def test_activity_endpoint_saves_and_returns_kcal(tmp_path, monkeypatch):
    """POST /api/activity saves Activity row and returns kcal"""
    ...
    from app.models import WeightLog, Activity
    ...
    with TestClient(app, follow_redirects=False) as client:
        ...
        client.post("/register", data={
            "email": "activity@test", "password": "tajnehaslo1",
            "password2": "tajnehaslo1", "invite_code": INVITE,
        })

        # Add a weight so endpoint doesn't fail
        db = SessionLocal()
        db.add(WeightLog(user_id=1, date=date.today(), weight_kg=70))
        db.commit()
        db.close()
        ...
        body = resp.json()
        assert body["kcal"] > 0
        assert body["weight_used_kg"] == 70.0

        # Verify Activity row was saved
        db = SessionLocal()
        activity_rows = db.scalars(select(Activity)).all()
        assert len(activity_rows) == 1
        assert activity_rows[0].source == "manual"
        assert activity_rows[0].kcal_manual == body["kcal"]
        db.close()
